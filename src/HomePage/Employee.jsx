...
export default function Employee() {
    ...
    const fetchEmployees = async () => {
        setLoading(true);
        try {
            const response = await fetch('http://52.228.61.110:8085/api/v1/employeeManager/employees');
            if (!response.ok) {
    ...
    const handleDeleteEmployee = async (employeeId) => {
        try {
            const response = await fetch(`http://52.228.61.110:8085/api/v1/employeeManager/employees/${employeeId}`, {
                method: 'DELETE',
            });
            if (response.ok) {
                fetchEmployees();
